...
export type DeskHealthState = "healthy" | "degraded" | "offline";

export type DeskHealth = {
  state: DeskHealthState;
  fetchedAt: string;
  newestWireAt: string | null;
  liveWireCount: number;
...
export function deskHealth(desk: DeskPayload, now = new Date()): DeskHealth {
  const rows = Object.values(desk.wires).flat();
  const liveRows = rows.filter((item) => Number.isFinite(Date.parse(item.published)));
  const newest = liveRows.reduce<number | null>((latest, item) => {
    const stamp = Date.parse(item.published);
    return latest === null || stamp > latest ? stamp : latest;
  }, null);
  const liveQuoteCount = desk.quotes.filter((quote) => quote.price > 0).length;
  const fetchAge = now.getTime() - Date.parse(desk.fetchedAt);
  const newestAge = newest === null ? Number.POSITIVE_INFINITY : now.getTime() - newest;
  ...
  return {
    ...
    newestWireAt: newest === null ? null : new Date(newest).toISOString(),
    liveWireCount: liveRows.length,
    totalWireCount: rows.length,
    liveQuoteCount,
  };
}
