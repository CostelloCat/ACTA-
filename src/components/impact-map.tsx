...
import { IMPACT_GRAPHS, type ImpactGraph, type ImpactNode, type ImpactNodeKind } from "@/lib/impact-map-data";
...
const KIND_DOT: Record<ImpactNodeKind, string> = {
  mechanism: "bg-rates",
  asset: "bg-gold",
  sector: "bg-mag7",
  company: "bg-inflation",
};
  ...
  return (
    <div>
      ...
      <div className="border-line bg-surface overflow-hidden rounded-2xl border">
        <div className="relative aspect-[16/10] w-full sm:aspect-[16/8]">
          ...
          <button
            ...
            className={cn(
              "border-line bg-raised absolute z-10 -translate-x-1/2 -translate-y-1/2 rounded-xl border px-2.5 py-2 text-left shadow-lg",
              "w-24 sm:w-28",
              selectedId === "event" && "ring-gold ring-2",
            )}
            style={{ left: `${positions.get("event")!.x}%`, top: `${positions.get("event")!.y}%` }}
          >
            ...
            col.nodes.map((node) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "border-line bg-raised absolute z-10 -translate-x-1/2 -translate-y-1/2 rounded-xl border px-2 py-1.5 text-left shadow-lg",
                    "w-24 sm:w-28",
                    selectedId === node.id && "ring-gold ring-2",
                    !active && selectedId !== "event" && "opacity-50",
                  )}
                  style={{ left: `${pos.x}%`, top: `${pos.y}%` }}
                >
                  <span className="flex items-center gap-1">
                    <i className={cn("size-1.5 shrink-0 rounded-full", KIND_DOT[node.kind])} />
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-[11px] tracking-wide uppercase">
        {COLUMN_ORDER.map((k) => (
          <span key={k} className="text-muted flex items-center gap-1.5">
            <i className={cn("size-2 rounded-full", KIND_DOT[k])} />
            {COLUMN_LABEL[k]}
          </span>
        ))}
      </div>
      ...
      <div className="border-line bg-surface mt-5 rounded-2xl border p-5">
        ...
        {selectedNode ? (
          <>
            ...
            <p className="text-muted mt-3 text-[10px] tracking-[0.22em] uppercase">Why it's connected</p>
      ...
      <p className="text-muted mt-4 text-xs leading-relaxed">
        ...
        signal. A production version would source and validate these links from event-study/correlation data, not
        static prose. The event card itself ({sourceEvent?.title ?? graph.eventTitle}) pulls its real WHAT/WHY text
        from the live{" "}
