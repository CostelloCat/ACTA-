...
function Desk() {
  ...
  const quotes = MARKET_PULSE.map((market) => ({
    ...market,
    quote: desk.quotes.find((quote) => quote.symbol === market.symbol),
  }));
    ...
    <Shell>
      <main className="mx-auto max-w-7xl">
          ...
          <aside className="grid gap-4">
            ...
            <section className="rounded-2xl border border-line bg-black/30 p-4">
              <div className="flex items-end justify-between gap-3">
                ...
                <Link to="/news" className="text-gold text-xs no-underline">
                  Radar →
                </Link>
              </div>
              <ol className="mt-3 divide-y divide-line">
                {signals.slice(1, 3).map((signal) => (
                  <li key={signal.id} className="py-3 first:pt-0 last:pb-0">
                    <p className="text-muted text-[9px] tracking-[0.16em] uppercase">
                ...
                {signals.length < 2 ? (
                  <li className="text-muted py-3 text-sm">
                    No additional source-linked signal is active.
                  </li>
                ) : null}
              </ol>
            </section>
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-muted text-[10px] tracking-[0.22em] uppercase">
                Tape · live when available
              </p>
              <h2 className="mt-1 text-2xl">Four markets, four different stories</h2>
            </div>
            <p className="text-muted text-xs">
              Updated {formatFetched(desk.fetchedAt)} · unavailable means unavailable
            </p>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {quotes.map((item) => (
              <MarketPulse key={item.symbol} {...item} />
            ))}
          </div>
        </section>
        ...
        <section className="mt-4 grid gap-3 sm:grid-cols-2">
          <Link
            to="/nq"
            className="group rounded-2xl border border-line bg-surface p-4 no-underline transition-colors hover:border-gold/50 sm:p-5"
          >
            <span className="text-gold text-[10px] tracking-[0.18em] uppercase">
              Trade the market
            </span>
            ...
            <span className="text-muted mt-2 block text-sm leading-relaxed">
              Start with the macro catalyst, then demand confirmation from rates, breadth, and the
              index.
            </span>
          </Link>
          <Link
            to="/mag7"
            className="group rounded-2xl border border-line bg-surface p-4 no-underline transition-colors hover:border-mag7/60 sm:p-5"
          >
            <span className="text-mag7 text-[10px] tracking-[0.18em] uppercase">
              Trade the name
            </span>
            <strong className="mt-1 block text-xl font-medium">Mag 7 / single stock</strong>
            <span className="text-muted mt-2 block text-sm leading-relaxed">
              Start with the company catalyst; use NQ and peer breadth as confirmation, not as the
              thesis.
            </span>
          </Link>
        </section>
    ...
    return (
      <section className="rounded-2xl border border-line bg-surface p-5">
        ...
        <p className="text-muted mt-3 text-sm">
          That means ACTA has no source-linked event to elevate—not that nothing is happening.
        </p>
        <Link to="/news" className="pill mt-5">
          Open Radar
        </Link>
      </section>
    );
    ...
    <article className="overflow-hidden rounded-2xl border border-line bg-surface">
      <div className="p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span
            className={cn(
              "rounded-full border px-2 py-1 text-[9px] tracking-wide uppercase",
              urgent ? "border-live/50 bg-live/15 text-red-200" : "border-gold/35 text-gold",
            )}
          >
            {statusLabel(signal)}
          </span>
        ...
        <p className="text-muted mt-2 text-[10px] tracking-[0.14em] uppercase">
          {signal.actor} · {signal.source} · {signalTimeLabel(signal)}
        </p>
      ...
      <div className="grid gap-px bg-line sm:grid-cols-2 xl:grid-cols-3">
        ...
        <BriefCell label="Direct tape first" value={lens.firstMove} />
        <BriefCell label="Benefits if confirmed" value={lens.beneficiaries} tone="text-up" />
        <BriefCell label="Under pressure" value={lens.pressure} tone="text-down" />
        <BriefCell label="Confirm with" value={lens.confirms} tone="text-up" />
        <BriefCell label="Thesis fails if" value={lens.invalidates} tone="text-down" />
      </div>
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line p-4">
        <p className="text-muted text-[10px] tracking-wide uppercase">
          Direct exposure · {signal.hits}
        </p>
        <a
          href={signal.href}
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        {signal ? (
          <span className="text-gold text-[10px] tabular-nums">{statusLabel(signal)}</span>
        ) : null}
      </div>
      {signal ? (
        <>
          ...
          <Link to="/calendar" className="pill mt-4">
            Prepare in Calendar →
          </Link>
        </>
      ) : (
...
function MarketPulse({
  ...
  return (
    <Link
      to={to}
      className="group rounded-xl border border-line bg-raised p-3 no-underline transition-colors hover:border-white/30"
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span
          className={cn(
            ...
            !live ? "text-muted" : quote!.changePct < 0 ? "text-down" : "text-up",
          )}
        >
