...
import { buildCatalystSignals, scheduledCatalysts, signalTimeLabel, statusLabel } from "@/lib/catalysts";
      ...
      <div className="mx-auto max-w-xl space-y-3 p-3">
        ...
        <section className="overflow-hidden rounded-2xl border border-line bg-surface">
          <div className="border-b border-line p-4">
            <p className="text-gold text-[10px] tracking-[0.2em] uppercase">Tape brief</p>
            <h1 className="mt-1 text-xl">Three things to know</h1>
            <p className="text-muted mt-1 text-xs">Catalyst, transmission, scheduled risk. Sources before stories.</p>
          </div>
          {active && lens ? (
            <article>
              <div className="border-b border-line p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-gold text-[10px] tracking-[0.2em] uppercase">01 · Priority now</p>
              ...
              <p className="text-muted mt-2 text-[10px] tracking-wide uppercase">{active.actor} · {active.source} · {signalTimeLabel(active)}</p>
              <p className="text-muted mt-3 text-sm leading-relaxed">{active.summary}</p>
              </div>

              <div className="grid gap-px bg-line sm:grid-cols-2">
                <DecisionCell label="02 · TRANSMISSION" value={`${active.mechanism}. First look: ${lens.firstMove}`} />
                <DecisionCell label="03 · CONFIRM / REJECT" value={`Confirm: ${lens.confirms} Reject: ${lens.invalidates}`} accent="text-up" />
              </div>

              <div className="flex items-center justify-between gap-3 border-t border-line p-3">
                <p className="text-muted text-[10px] tracking-wide uppercase">{lens.clock} · {active.hits}</p>
                <a href={active.href} target={active.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="pill pill-solid shrink-0">
                  Verify source {active.href.startsWith("http") ? "↗" : "→"}
                </a>
              </div>
            </article>
          ) : (
            <p className="p-5 text-sm text-muted">No source-linked catalyst is available. That means “no verified signal,” not “nothing is happening.”</p>
          )}
        </section>
        ...
        <section className="rounded-2xl border border-line bg-black/25 p-4">
          <p className="text-muted text-[9px] tracking-[0.2em] uppercase">Calendar · scheduled risk</p>
        ...
        <p className="text-muted px-1 pb-4 text-[10px] leading-relaxed">
          ACTA reports context, not certainty. Verify the source, price reaction, and risk limits before changing a live position.
        </p>
      </div>
    </main>
  );
