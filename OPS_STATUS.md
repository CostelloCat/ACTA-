...
- [ ] Restore intended iridescent / gold ACTA typography treatment from the stronger prior revision; avoid flat white where brand treatment should appear — **holding, need the reference**: "the stronger prior revision" isn't something Claude has access to (not in this repo's history, not the V1 zip, which is a different, lighter Apple-style theme — see PRODUCT.md). Please point to the specific asset/screenshot/commit so this isn't a guess at brand intent.
