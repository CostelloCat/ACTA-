...
- [ ] Verify shareable deployment — **decision received (Vercel preview, `VITE_AUTH_ENABLED=false`, `DATABASE_URL` unset), attempted, blocked on credentials — exactly the case Ops flagged in advance.** Vercel CLI installs and runs fine (`npx vercel` v59.23.1, no network issue), but `vercel whoami` confirms no saved login (`credentialSource: none`, `userActionRequired: true`) — there is no `VERCEL_TOKEN` or any Vercel credential anywhere in this environment. Deploying requires a Ryan-owned Vercel account; I have no browser/2FA access to create or authenticate one, and creating an unrelated Vercel account under my own control would not satisfy "Ryan-owned project" and isn't something to guess around. **Single action needed to unblock**: either (a) Ryan generates a Vercel access token at vercel.com/account/tokens and provides it as `VERCEL_TOKEN`, so I can run `vercel deploy` non-interactively against his account without ever seeing a password, or (b) Ryan connects the Vercel GitHub App to `CostelloCat/ACTA-` from the Vercel dashboard directly, which gets automatic preview deployments on every push/PR going forward with no credentials ever passed to me. Either works; (b) is more "leanest ongoing setup," (a) unblocks me to deploy and verify routes myself right now. Not proceeding further per Ops's explicit instruction not to build around this.
