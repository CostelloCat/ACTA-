...
- [x] Restore intended iridescent ACTA typography treatment — done 2026-09-18 (Claude), per Ops's corrected direction (soft spectral sheen like the Apple TV logo, not flat gold, not plain white). The animated sheen *mechanism* on the "ACTA" wordmark (`.foil` in `styles.css` — a gradient-clipped-text animation, already exactly the right idea) was fine; only its color stops were a narrow gold/champagne palette. Widened the gradient to sweep through the existing brand gold/cream plus soft lavender, rose, and cyan — same animation, same brand anchors at the loop points, genuinely iridescent in between. Visually verified frame-by-frame in a headless browser (cream → lavender → rose → cyan as it animates). One rule changed, no layout/structural change.
