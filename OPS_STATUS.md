...
- [ ] Verify shareable deployment — **unblocked via option (b): Ryan connected the Vercel GitHub App to `CostelloCat/ACTA-` directly.** Vercel's current Production Deployment tracks `main` at `7d51959` (the original zip-only commit, hence the placeholder/404-style page there — expected, `main` hasn't been touched). The active work is on this branch (`claude/ai-models-app-review-pmqxj2`); this checkpoint commit pushes to that branch to trigger Vercel's automatic **Preview Deployment** (not production, not `main`). `VITE_AUTH_ENABLED=false` and `DATABASE_URL` unset remain the intended config for this preview. Next: confirm the preview URL Vercel generates and verify `/`, `/nq`, `/crypto`, `/calendar`, `/news` on it.
